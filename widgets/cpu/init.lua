-- ~/.hammerspoon/widgets/cpu/init.lua
-- Mean CPU usage (average active residency across all cores), from the shared
-- powermetrics provider. Turns red above 80%. Mirrored across displays.
...
function M.start(widgets, opts)
    ...
    local canvases = widgets.mirror({ x = opts.x or 20, y = opts.y or 675, w = 110, h = opts.h or 48 }, function(c)
    ...
    local function render(cpu)
        local color = (cpu and cpu > threshold) and alert or normal
        local text  = cpu and string.format("CPU: %.0f %%", cpu) or "CPU: N/A"
    ...
    require("widgets.pmusage").subscribe(function(cpu, _) render(cpu) end)
